    ...
    def patch(user, info_id=None):
        ...
        if components:
            for component in components:
                new_total_price += component.price * component.amount
        if additional:
            for component in additional:
                new_total_price += component.price * component.amount
        if new_total_price != build.total_price:
            build.total_price = new_total_price
            db.session.commit()
        return {'total_price': new_total_price}, 201
