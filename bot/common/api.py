...
class Api:
    ...
    def user_exists(self, username: str) -> dict | None:
        response = requests.get(self.IP + f'/users?username={username}')
        if response.status_code == 200:
            return response.json()
        return None

    def add_new_user(self, username: str) -> dict | None:
    ...
    def init_pc(self, user_id: int = 1) -> dict | None:
    ...
    def get_title_and_id_list(self, user_id: int):
        return self.get_list(filters_str={'user_id': user_id}, params=['id', 'title'])
