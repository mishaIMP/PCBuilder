...
async def choose_mode(callback: types.CallbackQuery, state: FSMContext):
    filters = {'min_price': None, 'max_price': None, 'author': None, 'title': None}
...
async def choose_filters(callback: types.CallbackQuery, state: FSMContext, api):
    if callback.data == 'back':
        ...
        if not res['count']:
            await callback.message.edit_text(FindText.NOTHING_FOUND, reply_markup=Buttons.back_to_filters)
        else:
            pc = {}
            assembly_list = []
            amount = 0
            ...
            for i in range(res['count']):
                build = api.get_whole_pc(info_id=res['data'][i]['id'])
                if build:
...
def register_find_handlers(find_router: Dispatcher):
    find_router.message.register(get_min_and_max_price, FindState.get_min_price, F.text.regexp(r'\d+'))
    find_router.message.register(get_min_and_max_price, FindState.get_max_price, F.text.regexp(r'\d+'))
